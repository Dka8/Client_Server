...
3 библиотеки и 2 исполняемых файла:
	1 — Network_lib - 	предназначена для работы с сетью и является ОПП оберткой Berkeley Sockets.
		...
		3) SafeTypes.h - 	определ¤ем безопасные типы, которые можно безопасно
					передавать по сети
		4) SocketHandle.h - 	описывает какое внутреннее обозначение сокета
	...
	3 — Client_lib -	предназначена для осуществления функциональности сервера в пределах
		...
		2) ClientTCP.h -	объект данного типо создается при использование
					User Datagram Protocol для связи с сервером
		3) ClientUDP.h -	объект данного типо создается при использование
					Transmission Control Protocol для связи с сервером
-----------------------------------------------------------------------------------------------------
