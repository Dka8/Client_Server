...
3 библиотеки и 2 исполняемых файла:
	...
	3 — Client_lib -	предназначена для осуществления функциональности сервера в пределах
		...
		2) Protocol.h -		содержит базовый класс-интерфейс Protocol и два класса
					наследника UDP и TCP, в которых реализована логика работы с сервером
...
Замечания по дальнейшему совершенствованию проекта:
	1.	Использовать мультипоточность на стороне клиента,
		для распаралеливания разных протокол связи и выделить отдельный поток
		для консольных комманд (exit и т.п.)
