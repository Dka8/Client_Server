...
3 библиотеки и 2 исполняемых файла:
	1 — Network_lib - 	предназначена для работы с сетью и является ОПП оберткой Berkeley Sockets.
		...
		1) Config.hpp - 	определяет переменные среды в зависимости от используемой операционной системы
					для корректной работы системных сетевых библиотек библиотек
		2) SocketBE.hpp -	в зависимости от операциооной системы подключает необходимые заголовочные файлы.
	...
	2 — Server_lib - 	предназначена для осуществления функциональности сервера в пределах
				...
				Двухпоточное приложение. Дополнительный поток обрабатывает команды, вводимые через консоль.
	...
	3 — Client_lib -	предназначена для осуществления функциональности сервера в пределах
		...
		2) Protocol.h -		содержит базовый класс-интерфейс Protocol и два класса наследника UDP и TCP,
					в которых реализована логика работы с сервером в зависимости от выбранного протокола.
					Выбор протокола передачи осуществляется при запуске приложения Client_main,
					через аргументы консоли.
-----------------------------------------------------------------------------------------------------
...
Замечания по дальнейшему совершенствованию проекта:
	1.	Использовать мультипоточность на стороне клиента,
